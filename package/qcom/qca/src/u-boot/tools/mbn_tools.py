...
gzip_outputdir = "/bin/gzip"
lzma_outputdir = "/usr/bin/lzma"
...
#----------------------------------------------------------------------------
# V7 Regular Boot Header Class
#----------------------------------------------------------------------------
class Boot_Hdr_V7:
    def __init__(self, init_val):
        self.image_id = ImageType.NONE_IMG
        self.flash_parti_ver = FLASH_PARTI_VERSION_V7
...
def image_header(env, gen_dict,
    ...
    elif header_format == 'reg' and mbn_version == 7:
        boot_header = Boot_Hdr_V7(init_val = int('0x0',16))
        boot_header.image_id = gen_dict['IMAGE_KEY_IMAGE_ID']
...
def pboot_gen_elf(env, elf_in_file_name,
    ...
    if compress_method == "com" or compress_method == "lzma":
        is_com = True
    else:
        is_com = False
    ...
    if elf_out_file_name != None:
        ...
        # If hash table program header != found, make sure to include it
            ...
            if ((is_com == True) and  ((curr_phdr.p_type == LOAD_TYPE) or (curr_phdr.p_type ==  PHDR_ENTR_TYPE))):
                ...
                if(data_len > 0):
                    ...
                    if compress_method == "lzma" :
                        Comfile = "comfile" + str(i) + ".lzma"
                        compress_cmd = [lzma_outputdir, '-z', '-k', '-f',
                                str(srcfile),]
                    else:
                        Comfile = "comfile" + str(i) + ".gz"
                        compress_cmd = [gzip_outputdir, '-n', '-k', '-S.gz',
                                '-f', '-6', str(srcfile),]
                    ...
                    prc = subprocess.Popen(compress_cmd)
                    prc.wait()
    ...
    if elf_out_file_name != None:
        ...
        if (hash_seg_max_size != None):
            ...
            if (hash_seg_max_size & (ELF_BLOCK_ALIGN-1)) != 0:
                raise RuntimeError("Hash segment size passed != ELF Block Aligned: " + hex(hash_seg_max_size))
        ...
        # Append the flash type in ELF header e_type as os specific
        if ("norplusmmc" in elf_out_file_name):
            flash = 0x8
        elif ("norplusnand" in elf_out_file_name):
            flash = 0x7
        elif ("nand" in elf_out_file_name):
            flash = 0xB
        elif ("mmc" in elf_out_file_name):
            flash = 0x5
        elif ("nor" in elf_out_file_name):
            flash = 0x6
        else:
            flash = 0x0

        if (flash != 0x0):
            elf_header.e_type = flash | ET_LOOS
        ...
        for i in range(num_phdrs):
            ...
            #file_copy_offset(elf_in_fp, src_offset, elf_out_fp, curr_phdr.p_offset + hashtable_shift, curr_phdr.p_filesz)
                if(((curr_phdr.p_type == LOAD_TYPE) or (curr_phdr.p_type == PHDR_ENTR_TYPE))   and (curr_phdr.p_filesz > 0)) :
                    com_out_fp = OPEN(Comfile, "rb")
...
def modify_elf_flags(env, elf_in_file_name,
    ...
    if len(segment_list) != elf_header.e_phnum:
        ...
        for values in zip(values):
            ...
            if (new_entry['Define'] == '#define') and \
               ...
               (new_entry['Value'] != None):
                ...
                if global_dict != None and len(list(global_dict.keys())) > 0:
...
def filter_dictionary(env, global_dict, **kwargs):
    ...
    # If IMAGE_TYPE parameter != provided, raise error
    ...
    # If FLASH_TYPE parameter != provided, default to 'nand'
    ...
    # Check for MBN Type
    # If MBN_TYPE parameter != provided, default to 'elf'
    ...
    # Check for Image ID
    # If IMAGE_ID parameter != provided, default to ID 0
...
#----------------------------------------------------------------------------
def initialize_hash_phdr(elf_in_file_name, hash_tbl_size, hdr_size, hdr_offset, is_elf64, align_size = ELF_BLOCK_ALIGN):
    ...
    pad_hash_segment = (hash_tbl_end_addr) & (align_size-1)
    ...
    hash_Phdr.p_align = align_size
    hash_Phdr.p_offset = hash_hdr_offset
    if (pad_hash_segment):
        hash_Phdr.p_memsz = hash_hdr_size + hash_tbl_size + (align_size - pad_hash_segment)
    else:
        hash_Phdr.p_memsz = hash_hdr_size + hash_tbl_size
